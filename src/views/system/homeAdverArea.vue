<template>
  <div v-if="!editState" class="app-container calendar-list-container">
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column align="center" min-width="90" label="广告位置">
        <template slot-scope="scope">
          <span>{{scope.row.author}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="110" label="广告图片">
        <template slot-scope="scope">
          <span>{{scope.row.author}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="110" label="广告名称">
        <template slot-scope="scope">
          <span>{{scope.row.author}}</span>
        </template>
      </el-table-column>
  ...
  <div v-else-if="editState">
    <edit :data="editData" @homeAdverUpdate="hideEditState"></edit>
  </div>
</template>
...
<script>
  ...
  import edit from './components/homeAdverUpdate'
